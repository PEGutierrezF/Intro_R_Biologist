...
---
# Equality
- We can use the symbol `==` to test if two values are equal.
```{r}
'HELLO'=='GOOBYE'
```
...
---
# Inequality
- We can use the symbol `!=` to test if two values are not equal.
...
- We can use the symbols `>` (greater than) and `<` (less than) to compare values.
```{r}
5>6
```
...
---
# Simple math in R
- R can be used as a calculator. You can perform basic arithmetic operations directly in the console or in code chunks.
```{r}
5 + 5
```
...
- You can use variables (objects) in calculations just like you would with numbers.
```{r}
var1 <- 10
var2 <- 20
var3 <- var1 + var2
...
- We can check the type of an object using the `class()` function.
```{r}
class(var3)
class(my_obj2)
```
